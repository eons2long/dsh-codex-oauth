  ...
  ctx.inject(["webServer"], (web) => ctx.effect(() => {
    const routes = [
      ...
      web.webServer.register({ kind: "exact", path: AUTH_LOGIN_PATH, handler: async (req, res) => {
        if (!trusted(req) || !["GET", "POST"].includes(req.method)) return json(res, req.method === "GET" || req.method === "POST" ? 403 : 405, { error: "forbidden" });
        try {
          start();
          ...
          if (req.method === "GET") { res.writeHead(302, { location: challenge.url, "cache-control": "no-store" }); res.end(); }
          else json(res, 200, challenge);
