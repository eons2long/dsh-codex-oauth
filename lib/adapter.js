    ...
    auth: {
      ...
      apiKey: {
        ...
        async resolve({ credential }) {
          const access = credential?.access ?? credential?.key;
          return access
            ? { auth: { apiKey: access }, source: "DSH Codex OAuth" }
            : undefined;
        },
      },
    },
...
export function createCodexAdapter(credentials, resolveAttachments) {
  ...
  const profiles = new Map([[OPENAI_CODEX_PROVIDER, {
    ...
    retryPolicy: resolveRetryPolicy(undefined, "dsh-codex-oauth retryPolicy"),
